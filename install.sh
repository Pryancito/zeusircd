...
mac()
{
	...
	qpid
	for program in nano gcc cpp gcc-c++ openssl sqlite git gmake libicu gettext libmaxminddb mysql yaml-cpp qpid-proton qpid-cpp; do brew install $program; done
