	...
	virtual void command(LocalUser *user, std::string message) override {
		...
		if (cmd == "HELP") {
			...
			if (split.size() < 2) {
				...
				user->Send(":" + config["chanserv"].as<std::string>() + " NOTICE " + user->mNickName + " :" + Utils::make_string(user->mLang, "You need to be into the channel and got @ to make %s.", "DROP"));
				return;
			} else if (ChanServ::IsFounder(user->mNickName, split[1]) == false) {
				user->Send(":" + config["chanserv"].as<std::string>() + " NOTICE " + user->mNickName + " :" + Utils::make_string(user->mLang, "You are not the founder of the channel."));
				return;
			} else {
				string sql = "DELETE FROM CANALES WHERE NOMBRE='" + split[1] + "';";
				if (DB::SQLiteNoReturn(sql) == false) {
					user->Send(":" + config["chanserv"].as<std::string>() + " NOTICE " + user->mNickName + " :" + Utils::make_string(user->mLang, "The channel %s cannot be deleted. Please contact with an iRCop.", split[1].c_str()));
					return;
				}
				if (config["database"]["cluster"].as<bool>() == false) {
					sql = "DB " + DB::GenerateID() + " " + sql;
				...
				user->Send(":" + config["chanserv"].as<std::string>() + " NOTICE " + user->mNickName + " :" + Utils::make_string(user->mLang, "The channel %s has been deleted.", split[1].c_str()));
				Channel* chan = Mainframe::instance()->getChannelByName(split[1]);
				if (chan) {
			...
			if (split.size() < 3) {
				...
				user->Send(":" + config["chanserv"].as<std::string>() + " NOTICE " + user->mNickName + " :" + Utils::make_string(user->mLang, "The channel %s is not registered.", split[1].c_str()));
				return;
			} else if (Server::HUBExiste() == false) {
				user->Send(":" + config["chanserv"].as<std::string>() + " NOTICE " + user->mNickName + " :" + Utils::make_string(user->mLang, "The HUB doesnt exists, DBs are in read-only mode."));
				return;
				...
				user->Send(":" + config["chanserv"].as<std::string>() + " NOTICE " + user->mNickName + " :" + Utils::make_string(user->mLang, "You do not have enough access."));
				return;
			} else {
				if (strcasecmp(split[2].c_str(), "ADD") == 0) {
					if (split.size() < 4) {
						...
						user->Send(":" + config["chanserv"].as<std::string>() + " NOTICE " + user->mNickName + " :" + Utils::make_string(user->mLang, "The nick %s is not registered.", split[3].c_str()));
						return;
					} else if (NickServ::GetOption("NOACCESS", split[3]) == true) {
						user->Send(":" + config["chanserv"].as<std::string>() + " NOTICE " + user->mNickName + " :" + Utils::make_string(user->mLang, "The option NOACCESS is enabled for this user."));
						return;
					}
					if (ChanServ::Access(split[3], split[1]) != 0) {
						...
						user->Send(":" + config["chanserv"].as<std::string>() + " NOTICE " + user->mNickName + " :" + Utils::make_string(user->mLang, "The nickname has already access: %s", acceso.c_str()));
						return;
					} else {
						string sql = "INSERT INTO ACCESS VALUES ('" + split[1] + "', '" + cmd + "', '" + split[3] + "', '" + user->mNickName + "');";
						if (DB::SQLiteNoReturn(sql) == false) {
							user->Send(":" + config["chanserv"].as<std::string>() + " NOTICE " + user->mNickName + " :" + Utils::make_string(user->mLang, "Cannot insert the record."));
							return;
						}
						if (config["database"]["cluster"].as<bool>() == false) {
							sql = "DB " + DB::GenerateID() + " " + sql;
						...
						user->Send(":" + config["chanserv"].as<std::string>() + " NOTICE " + user->mNickName + " :" + Utils::make_string(user->mLang, "The record has been inserted."));
						LocalUser *target = Mainframe::instance()->getLocalUserByName(split[3]);
						if (target)
							ChanServ::CheckModes(target, split[1]);
					}
					user->Send(":" + config["chanserv"].as<std::string>() + " NOTICE " + user->mNickName + " :" + Utils::make_string(user->mLang, "%s has been given to %s", cmd.c_str(), split[3].c_str()));
					...
					if (ChanServ::Access(split[3], split[1]) == 0) {
						user->Send(":" + config["chanserv"].as<std::string>() + " NOTICE " + user->mNickName + " :" + Utils::make_string(user->mLang, "The user do not have access."));
						return;
					}
					string sql = "DELETE FROM ACCESS WHERE USUARIO='" + split[3] + "'  AND CANAL='" + split[1] + "'  AND ACCESO='" + cmd + "';";
					if (DB::SQLiteNoReturn(sql) == false) {
						user->Send(":" + config["chanserv"].as<std::string>() + " NOTICE " + user->mNickName + " :" + Utils::make_string(user->mLang, "The record cannot be deleted."));
						return;
					}
					if (config["database"]["cluster"].as<bool>() == false) {
						sql = "DB " + DB::GenerateID() + " " + sql;
					...
					if (target)
						ChanServ::CheckModes(target, split[1]);
					user->Send(":" + config["chanserv"].as<std::string>() + " NOTICE " + user->mNickName + " :" + Utils::make_string(user->mLang, "%s has been removed to %s", cmd.c_str(), split[3].c_str()));
					...
					if (result.size() == 0) {
						user->Send(":" + config["chanserv"].as<std::string>() + " NOTICE " + user->mNickName + " :" + Utils::make_string(user->mLang, "There is no access of %s", cmd.c_str()));
						return;
					}
					for(vector<vector<string> >::iterator it = result.begin(); it < result.end(); ++it)
					{
						vector<string> row = *it;
						user->Send(":" + config["chanserv"].as<std::string>() + " NOTICE " + user->mNickName + " :" + Utils::make_string(user->mLang, "\002%s\002 accessed by %s", row.at(0).c_str(), row.at(1).c_str()));
					}
				}
				return;
			}
			...
			if (split.size() < 3) {
				...
				if (topic.length() > 250) {
					user->Send(":" + config["chanserv"].as<std::string>() + " NOTICE " + user->mNickName + " :" + Utils::make_string(user->mLang, "The topic is too long."));
					return;
				}
				string sql = "UPDATE CANALES SET TOPIC='" + encode_base64((const unsigned char*)topic.c_str(), topic.length()) + "' WHERE NOMBRE='" + split[1] + "';";
				if (DB::SQLiteNoReturn(sql) == false) {
					user->Send(":" + config["chanserv"].as<std::string>() + " NOTICE " + user->mNickName + " :" + Utils::make_string(user->mLang, "The topic can not be changed."));
					return;
				}
				if (config["database"]["cluster"].as<bool>() == false) {
					sql = "DB " + DB::GenerateID() + " " + sql;
				...
				user->Send(":" + config["chanserv"].as<std::string>() + " NOTICE " + user->mNickName + " :" + Utils::make_string(user->mLang, "The topic has changed."));
				return;
			}
		} else if (cmd == "AKICK") {
			if (split.size() < 3) {
				...
				if (strcasecmp(split[2].c_str(), "ADD") == 0) {
					...
					if (ChanServ::IsAKICK(split[3], split[1]) != 0) {
						user->Send(":" + config["chanserv"].as<std::string>() + " NOTICE " + user->mNickName + " :" + Utils::make_string(user->mLang, "The mask already got AKICK."));
						return;
					} else {
						...
						if (DB::EscapeChar(motivo) == true || DB::EscapeChar(split[3]) == true) {
							user->Send(":" + config["chanserv"].as<std::string>() + " NOTICE " + user->mNickName + " :" + Utils::make_string(user->mLang, "The reason or the mask contains no-valid characters."));
							return;
						}
						string sql = "INSERT INTO AKICK VALUES ('" + split[1] + "', '" + split[3] + "', '" + motivo + "', '" + user->mNickName + "');";
						if (DB::SQLiteNoReturn(sql) == false) {
							user->Send(":" + config["chanserv"].as<std::string>() + " NOTICE " + user->mNickName + " :" + Utils::make_string(user->mLang, "The AKICK can not be inserted."));
							return;
						}
						if (config["database"]["cluster"].as<bool>() == false) {
							sql = "DB " + DB::GenerateID() + " " + sql;
					...
					if (ChanServ::IsAKICK(split[3], split[1]) == 0) {
						user->Send(":" + config["chanserv"].as<std::string>() + " NOTICE " + user->mNickName + " :" + Utils::make_string(user->mLang, "The user do not have AKICK."));
						return;
					}
					...
					user->Send(":" + config["chanserv"].as<std::string>() + " NOTICE " + user->mNickName + " :" + Utils::make_string(user->mLang, "The AKICK for %s has been deleted.", split[3].c_str()));
				} else if (strcasecmp(split[2].c_str(), "LIST") == 0) {
					vector<vector<string> > result;
					string sql = "SELECT MASCARA, ADDED, MOTIVO FROM AKICK WHERE CANAL='" + split[1] + "' ORDER BY MASCARA;";
					result = DB::SQLiteReturnVectorVector(sql);
					if (result.size() == 0) {
						user->Send(":" + config["chanserv"].as<std::string>() + " NOTICE " + user->mNickName + " :" + Utils::make_string(user->mLang, "AKICK list of channel %s is empty.", split[1].c_str()));
						return;
					}
					for(vector<vector<string> >::iterator it = result.begin(); it < result.end(); ++it)
					{
						vector<string> row = *it;
						user->Send(":" + config["chanserv"].as<std::string>() + " NOTICE " + user->mNickName + " :" + Utils::make_string(user->mLang, "\002%s\002 updated by %s. Reason: %s", row.at(0).c_str(), row.at(1).c_str(), row.at(2).c_str()));
					}
				}
				return;
			}
			...
			if (split.size() < 3) {
				...
				user->Send(":" + config["serverName"].as<std::string>() + " 401 " + user->mNickName + " :" + Utils::make_string(user->mLang, "The nick does not exist."));
				return;
			} else {
				char modo;
				bool action;
				...
				if (!target)
				{
					...
					if (chan->isVoice(target)) {
						if (modo == 'h' && action == 1) {
							...
							user->Send(":" + config["chanserv"].as<std::string>() + " NOTICE " + user->mNickName + " :" + Utils::make_string(user->mLang, "The nick already got %s.", "VOICE"));
						...
						if (modo == 'v' && action == 1) {
							...
							user->Send(":" + config["chanserv"].as<std::string>() + " NOTICE " + user->mNickName + " :" + Utils::make_string(user->mLang, "The nick already got %s.", "HALFOP"));
						...
						if (modo == 'v' && action == 1) {
							...
							user->Send(":" + config["chanserv"].as<std::string>() + " NOTICE " + user->mNickName + " :" + Utils::make_string(user->mLang, "The nick already got %s.", "OP"));
						...
						if (modo == 'v' && action == 1) {
							...
							user->Send(":" + config["chanserv"].as<std::string>() + " NOTICE " + user->mNickName + " :" + Utils::make_string(user->mLang, "The nick do not have any mode."));
						}
					}
				} else {
			...
			if (split.size() < 3) {
				...
				if (DB::EscapeChar(key) == true) {
					user->Send(":" + config["chanserv"].as<std::string>() + " NOTICE " + user->mNickName + " :" + Utils::make_string(user->mLang, "The key contains no-valid characters."));
					return;
				}
				if (key.length() > 32) {
					user->Send(":" + config["chanserv"].as<std::string>() + " NOTICE " + user->mNickName + " :" + Utils::make_string(user->mLang, "The key is too long."));
					return;
				}
				if (strcasecmp(key.c_str(), "OFF") == 0) {
					string sql = "UPDATE CANALES SET CLAVE='' WHERE NOMBRE='" + split[1] + "';";
					if (DB::SQLiteNoReturn(sql) == false) {
						user->Send(":" + config["chanserv"].as<std::string>() + " NOTICE " + user->mNickName + " :" + Utils::make_string(user->mLang, "The key can not be changed."));
						return;
					}
					if (config["database"]["cluster"].as<bool>() == false) {
						sql = "DB " + DB::GenerateID() + " " + sql;
					...
					user->Send(":" + config["chanserv"].as<std::string>() + " NOTICE " + user->mNickName + " :" + Utils::make_string(user->mLang, "The key has been removed.", key.c_str()));
				} else {
					...
					user->Send(":" + config["chanserv"].as<std::string>() + " NOTICE " + user->mNickName + " :" + Utils::make_string(user->mLang, "The key has changed to: %s", key.c_str()));
				}
			}
		} else if (cmd == "MODE") {
			if (split.size() < 3) {
				...
				if (strcasecmp("LIST", split[2].c_str()) == 0) {
					user->Send(":" + config["chanserv"].as<std::string>() + " NOTICE " + user->mNickName + " :" + Utils::make_string(user->mLang, "The available modes are: flood, onlyreg, autovoice, moderated, onlysecure, nonickchange, onlyweb, country, onlyaccess"));
					return;
					...
					user->Send(":" + config["chanserv"].as<std::string>() + " NOTICE " + user->mNickName + " :" + Utils::make_string(user->mLang, "Unknown mode."));
					return;
				} if (split[2][0] == '+') {
					std::string sql;
					if (ChanServ::HasMode(split[1], mode) == true) {
						user->Send(":" + config["chanserv"].as<std::string>() + " NOTICE " + user->mNickName + " :" + Utils::make_string(user->mLang, "The channel %s already got the mode %s", split[1].c_str(), mode.c_str()));
						return;
					} if ((mode == "COUNTRY" || mode == "FLOOD") && split.size() != 4) {
						user->Send(":" + config["chanserv"].as<std::string>() + " NOTICE " + user->mNickName + " :" + Utils::make_string(user->mLang, "The flood mode got parameters."));
						return;
					} else if (mode == "FLOOD" && (!Utils::isnum(split[3]) || stoi(split[3]) < 1 || stoi(split[3]) > 999)) {
						user->Send(":" + config["chanserv"].as<std::string>() + " NOTICE " + user->mNickName + " :" + Utils::make_string(user->mLang, "The parameter of flood mode is incorrect."));
						return;
					} else if (mode == "FLOOD")
					...
					if (DB::SQLiteNoReturn(sql) == false) {
						user->Send(":" + config["chanserv"].as<std::string>() + " NOTICE " + user->mNickName + " :" + Utils::make_string(user->mLang, "The mode can not be setted."));
						return;
					}
					if (config["database"]["cluster"].as<bool>() == false) {
						sql = "DB " + DB::GenerateID() + " " + sql;
					...
					user->Send(":" + config["chanserv"].as<std::string>() + " NOTICE " + user->mNickName + " :" + Utils::make_string(user->mLang, "The mode is setted."));
					return;
				} else if (split[2][0] == '-') {
					if (ChanServ::HasMode(split[1], mode) == false) {
						user->Send(":" + config["chanserv"].as<std::string>() + " NOTICE " + user->mNickName + " :" + Utils::make_string(user->mLang, "The channel %s doesnt got the mode %s", split[1].c_str(), mode.c_str()));
						return;
					}
					string sql;
					if (mode == "COUNTRY")
					...
					if (DB::SQLiteNoReturn(sql) == false) {
						user->Send(":" + config["chanserv"].as<std::string>() + " NOTICE " + user->mNickName + " :" + Utils::make_string(user->mLang, "The mode cannot be removed."));
						return;
					}
					if (config["database"]["cluster"].as<bool>() == false) {
						sql = "DB " + DB::GenerateID() + " " + sql;
					...
					user->Send(":" + config["chanserv"].as<std::string>() + " NOTICE " + user->mNickName + " :" + Utils::make_string(user->mLang, "The mode has been removed."));
					return;
				}
			}
			...
			if (split.size() < 3) {
				...
				user->Send(":" + config["chanserv"].as<std::string>() + " NOTICE " + user->mNickName + " :" + Utils::make_string(user->mLang, "The target nick is not registered."));
				return;
			} else if (Server::HUBExiste() == false) {
				user->Send(":" + config["chanserv"].as<std::string>() + " NOTICE " + user->mNickName + " :" + Utils::make_string(user->mLang, "The HUB doesnt exists, DBs are in read-only mode."));
				return;
				...
				user->Send(":" + config["chanserv"].as<std::string>() + " NOTICE " + user->mNickName + " :" + Utils::make_string(user->mLang, "You do not have enough access to change founder."));
				return;
			} else {
				string sql = "UPDATE CANALES SET OWNER='" + split[2] + "' WHERE NOMBRE='" + split[1] + "';";
				if (DB::SQLiteNoReturn(sql) == false) {
					user->Send(":" + config["chanserv"].as<std::string>() + " NOTICE " + user->mNickName + " :" + Utils::make_string(user->mLang, "The founder cannot be changed."));
					return;
				}
				if (config["database"]["cluster"].as<bool>() == false) {
					sql = "DB " + DB::GenerateID() + " " + sql;
				...
				user->Send(":" + config["chanserv"].as<std::string>() + " NOTICE " + user->mNickName + " :" + Utils::make_string(user->mLang, "The founder has changed to: %s", split[2].c_str()));
				return;
			}
		}
	}
...
extern "C" Widget* factory(void) {
	return static_cast<Widget*>(new CMD_ChanServ);
}
