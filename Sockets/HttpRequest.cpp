...
DEB(	std::cout << " setup http form\n";)
	m_form = std::shared_ptr<HttpdForm>(new HttpdForm(fil));
}
#endif
...
{
	if (!m_body_file.get())
		m_body_file = std::shared_ptr<IFile>(new MemFile);
DEB(	else
...
void HttpRequest::ParseBody()
{
	...
	if ( (it = m_attribute.find("query_string")) != m_attribute.end())
	{
		...
		m_form = std::shared_ptr<HttpdForm>(new HttpdForm( qs, qs.size() ));
	}
	else
	if (m_body_file.get())
	{
		m_form = std::shared_ptr<HttpdForm>(new HttpdForm( m_body_file.get(), ContentType(), ContentLength() ));
	}
	else
	{
		// dummy
		m_form = std::shared_ptr<HttpdForm>(new HttpdForm( "", 0 ));
	}
}
...
void HttpRequest::Reset()
{
	...
	m_body_file = std::shared_ptr<IFile>(NULL);
	m_form = std::shared_ptr<HttpdForm>(NULL);
	m_cookies.Reset();
