...
public:
  ListenWSS(std::string ip, int port)
	...
	acceptor_(io_context_pool_.get_io_context(), boost::asio::ip::tcp::endpoint(boost::asio::ip::address::from_string(ip), port)),
...
class Channel
{
  public:
	...
	static bool FindChannel(const std::string nombre);
