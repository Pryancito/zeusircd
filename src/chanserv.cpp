		...
		if (split.size() < 3) {
			...
			if (chan->isVoice(target)) {
				if (modo == 'h' && action == 1) {
					...
					Server::sendall("CMODE " + config->Getvalue("chanserv") + " " + chan->name() + " -v " + target->mNickName);
					chan->broadcast(":" + config->Getvalue("chanserv") + " MODE " + chan->name() + " +h " + target->mNickName);
					chan->giveHalfOperator(target);
					Server::sendall("CMODE " + config->Getvalue("chanserv") + " " + chan->name() + " +h " + target->mNickName);
...
void ChanServ::CheckModes(LocalUser *user, const string &channel) {
	...
	if (chan->isVoice(user) == true) {
		if (access < 1) {
			...
			chan->delVoice(user);
			Server::sendall("CMODE " + config->Getvalue("chanserv") + " " + chan->name() + " -v " + user->mNickName);
		} else if (access == 2) {
			chan->broadcast(":" + config->Getvalue("chanserv") + " MODE " + chan->name() + " -v " + user->mNickName);
			chan->delVoice(user);
			Server::sendall("CMODE " + config->Getvalue("chanserv") + " " + chan->name() + " -v " + user->mNickName);
			...
			Server::sendall("CMODE " + config->Getvalue("chanserv") + " " + chan->name() + " +h " + user->mNickName);
		} else if (access > 2) {
			chan->broadcast(":" + config->Getvalue("chanserv") + " MODE " + chan->name() + " -v " + user->mNickName);
			chan->delVoice(user);
			Server::sendall("CMODE " + config->Getvalue("chanserv") + " " + chan->name() + " -v " + user->mNickName);
			...
			Server::sendall("CMODE " + config->Getvalue("chanserv") + " " + chan->name() + " +o " + user->mNickName);
		}
