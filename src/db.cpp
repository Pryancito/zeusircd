...
void DB::IniciarDB () {
    ...
    if (DB::SQLiteNoReturn(sql) == false) {
   	...
   	if (config->Getvalue("dbtype") == "mysql")
		sql = "CREATE TABLE IF NOT EXISTS OPTIONS (NICKNAME VARCHAR(255) UNIQUE NOT NULL, NOACCESS INT DEFAULT 0, SHOWMAIL INT DEFAULT 0, NOMEMO INT DEFAULT 0, NOOP INT DEFAULT 0, ONLYREG INT DEFAULT 0, LANG TEXT );";
   	else
	...
	if (config->Getvalue("dbtype") == "mysql")
		sql = "CREATE TABLE IF NOT EXISTS CMODES (CANAL VARCHAR(255) UNIQUE NOT NULL, FLOOD INT DEFAULT 0, ONLYREG INT DEFAULT 0, AUTOVOICE INT DEFAULT 0, MODERATED INT DEFAULT 0, ONLYSECURE INT DEFAULT 0, NONICKCHANGE INT DEFAULT 0, ONLYWEB INT DEFAULT 0 );";
	else
