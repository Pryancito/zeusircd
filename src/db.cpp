...
void DB::initSQL() {
	...
	options.password = config["database"]["pass"].as<std::string>();
	options.dbname = config["database"]["db"].as<std::string>();
	options.timeout = 30;
...
int DB::Sync(Server *server, const std::string &id) {
...
std::string DB::GetLastRecord () {
	...
	std::string version = DB::SQLiteReturnString(sql);
	if (version == "")
		return "0";
	else
		return version;
